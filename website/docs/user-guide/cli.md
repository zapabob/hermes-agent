...
hermes worktree prune                 # print the reclaim plan; no changes
hermes worktree prune --yes           # apply the plan and remove approved paths
hermes worktree prune --apply         # same explicit consent as --yes
...
`--trees-only` and `--branches-only` are mutually exclusive. The bare
`hermes worktree prune` command is always plan-only; `--yes` or `--apply` is
required to perform the deletion. Inside a session, `/worktree prune
[--dry-run]` follows the same explicit-consent rule (and never touches the
