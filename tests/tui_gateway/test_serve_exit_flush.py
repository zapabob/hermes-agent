...
def test_sigterm_flushes_populated_session_into_state_db(
    ...
    try:
        ...
        if os.name == "nt":
            # Windows os.kill(SIGTERM) calls TerminateProcess and bypasses
            # Python signal handlers entirely. Exercise the installed handler
            # directly; POSIX keeps the real signal-delivery assertion.
