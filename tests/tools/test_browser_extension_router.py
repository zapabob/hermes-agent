...
_REAL_EXTENSION_TRANSPORT_POLICY_ERROR = browser_extension_router._extension_transport_policy_error
_REAL_LEGACY_TRANSPORT_POLICY_ERROR = browser_extension_router._legacy_browser_transport_policy_error


@pytest.fixture(autouse=True)
def _exercise_extension_route_below_the_production_policy(monkeypatch):
    monkeypatch.setattr(browser_extension_router, "_extension_transport_policy_error", lambda _action: "")
    monkeypatch.setattr(browser_extension_router, "_legacy_browser_transport_policy_error", lambda _action: "")


def test_registry_browser_dispatch_fails_closed_before_legacy_or_extension_routing(monkeypatch):
    monkeypatch.setattr(
        browser_extension_router,
        "_legacy_browser_transport_policy_error",
        _REAL_LEGACY_TRANSPORT_POLICY_ERROR,
    )
    fallbacks = []
...
def test_selected_extension_controller_fails_closed_before_dispatch(monkeypatch):
    broker = FakeBroker(scope="scope", selected=object())
    monkeypatch.setattr(
        browser_extension_router,
        "_extension_transport_policy_error",
        _REAL_EXTENSION_TRANSPORT_POLICY_ERROR,
    )
    ...
    assert result["blocked_by_policy"]["backend"] == "browser-extension"
