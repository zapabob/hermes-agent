...
--ignore-scripts (AGENT_BROWSER_NPX_SPEC is an exact audited release and pin),
and must kill the whole process tree — not just the top-level
npx PID — on timeout.
"""
...
def test_uses_new_process_group_creationflag_on_windows_instead_of_start_new_session():
    ...
    with patch("tools.browser_tool._is_windows", return_value=True), \
...
class TestLegacyKillProcessTree:
    ...
    def test_posix_kills_process_group_term_then_kill(self, monkeypatch):
        ...
        assert killpg_calls == [
            (999, signal.SIGTERM),
            (999, getattr(signal, "SIGKILL", signal.SIGTERM)),
        ]
