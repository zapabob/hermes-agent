...
class TestRealProfileCdpLaunch:
    ...
    def _no_live_agent_browser_probe(self, monkeypatch):
        import tools.browser_tool as bt

        monkeypatch.setattr(bt, "_agent_browser_get_cdp", lambda _session: None)
    ...
    def test_launch_returns_http_cdp(self, tmp_path):
        ...
        with patch.object(bt, "_use_real_profile", return_value=True), \
             ...
             patch.object(bt, "_ensure_agent_browser_runtime", return_value=True), \
...
class TestReviewRound3:
    ...
    def test_processes_holding_profile_identity_binding(self, tmp_path, monkeypatch):
        ...
        procs = [
            ...
            FakeProc("chrome-helper.exe", ["chrome-helper.exe", f"--user-data-dir={ud}"]),
            FakeProc("chrome.exe", ["chrome.exe", f"--user-data-dir={ud}-other"]),
    ...
    def test_ambiguous_browser_identity_never_terminates(self, tmp_path, monkeypatch):
        ...
        class FakeProc:
            info = {"name": "chrome.exe", "cmdline": None}
            terminated = False
        ...
        class FakePsutil:
            ...
            def wait_procs(self, *args, **kwargs):
                raise AssertionError("ambiguous identity must fail before termination")

        import sys as _sys
        monkeypatch.setitem(_sys.modules, "psutil", FakePsutil())
        closed, msg = bc.close_browser_holding_profile(str(tmp_path))
        assert closed is False
        assert "ambiguous" in msg
        assert proc.terminated is False

    def test_approved_exact_profile_process_closes(self, tmp_path, monkeypatch):
        import hermes_cli.browser_connect as bc
        ...
        ud = str(tmp_path / "exact-profile")
        parent = FakeProc("chrome.exe", ["chrome.exe", f"--user-data-dir={ud}"])
        child = FakeProc("chrome.exe", ["chrome.exe", "--type=renderer"])
        parent.child = child
        ...
        import sys as _sys
        monkeypatch.setitem(_sys.modules, "psutil", FakePsutil())
        monkeypatch.setattr(bc, "_last_used_profile", lambda _src: "Default")
        monkeypatch.setattr(bc, "_profile_is_locked", lambda _src, _profile: False)
        closed, msg = bc.close_browser_holding_profile(ud)
