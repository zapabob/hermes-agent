...
class TestPollingHealthConfirmation:
    ...
    def test_subsequent_progress_is_silent(self, caplog):
        ...
        # The first call establishes the generation's one confirmation log;
        # isolate the assertion below to the subsequent calls so caplog does
        # not mistake that intentional setup record for another emission.
        caplog.clear()
        with caplog.at_level(logging.INFO, logger="plugins.platforms.telegram.adapter"):
