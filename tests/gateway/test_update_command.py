...
class TestHandleUpdateCommand:
    ...
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m hermes_cli when hermes not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        # The update_cmd uses sys.executable -m hermes_cli
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "hermes_cli" in joined or "bash" in call_args[0]
