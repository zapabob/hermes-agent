...
def test_rearm_counts_system_prompt_overhead_for_tiny_transcript():
    """A short transcript must stay armed when the cached system prefix is
    itself larger than the model window."""
    agent = _FakeUncompressedAgent(context_length=1_000)
    agent._emit_warning = MagicMock()
    agent._last_ctx_overflow_warn = ("uncompressed_ctx_overflow", 1_000)
    agent._cached_system_prompt = "system instructions " * 1_000

    tctx = _build(agent, conversation_history=[
        {"role": "user", "content": "small"},
    ])

    assert isinstance(tctx, TurnContext)
    assert agent._last_ctx_overflow_warn == (
        "uncompressed_ctx_overflow",
        1_000,
    )


def test_rearm_counts_tool_schema_overhead_for_tiny_transcript():
    """Large tool schemas are part of the request even when messages are tiny."""
    agent = _FakeUncompressedAgent(context_length=1_000)
    agent._emit_warning = MagicMock()
    agent._last_ctx_overflow_warn = ("uncompressed_ctx_overflow", 1_000)
    agent.tools = [{
        ...
        "function": {
            "name": "large_tool",
            "description": "schema detail " * 2_000,
...
def test_none_content_tool_call_rows_are_included_in_real_rearm_estimate():
    """Assistant tool-call rows with content=None remain valid input to the
    overhead-aware estimator used by the re-arm gate."""
