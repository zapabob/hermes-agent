...
def test_current_config_keeps_stale_key_visible_to_doctor(tmp_path):
    """A post-cutover config is not silently treated as Relay-disabled."""
    config_path = tmp_path / "config.yaml"
    original = {
        "_config_version": 38,
        "plugins": {"enabled": ["observability/nemo_relay", "keep-me"]},
    }
    config_path.write_text(
