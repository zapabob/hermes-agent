...
def test_nul_and_line_structure_are_stripped():
    _write_env_vars({"MEM0_API_KEY": "key1\x00\r\nEVIL=injected\n"})

    env = load_env()
    assert env["MEM0_API_KEY"] == "key1EVIL=injected"
    assert "EVIL" not in _env_file_keys()
    assert "\x00" not in get_env_path().read_text(encoding="utf-8")


def test_filesystem_failure_is_not_reported_as_success(monkeypatch):
    import hermes_cli.config as config

    def _fail(_key, _value):
        raise PermissionError("read-only credential store")

    monkeypatch.setattr(config, "save_env_value", _fail)
    with pytest.raises(PermissionError, match="read-only credential store"):
        _write_env_vars({"MEM0_API_KEY": "m0-test-key-abc"})
