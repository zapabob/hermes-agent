...
def test_fallback_provider_counts_as_explicit(tmp_path, monkeypatch):
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "active_provider": None})
    _write_config(
        tmp_path,
        {
            "model": {"provider": "anthropic", "default": "claude-opus-4-8"},
            "fallback_providers": [
                {"provider": "hypura", "model": "local-model"},
            ],
        },
    )
    ...
    assert is_provider_explicitly_configured("hypura") is True
    assert is_provider_explicitly_configured("llama-cpp") is False
