...
def _patch_managed_uv(request):
    ...
    def _fake_resolve_node_runtime_npm():
        # Keep the legacy shutil.which mocks below authoritative while the
        # production resolver also searches managed Node installations.
        # Mirror its WSL safety rule so a mocked Windows PATH shim is not
        # accidentally accepted as a runnable Linux npm.
        npm = shutil.which("npm")
        if npm and not hm._is_windows() and hm._is_windows_npm_path(npm):
            return None
        return npm
    ...
    with patch("hermes_cli.managed_uv.resolve_uv", side_effect=_fake_resolve_uv), \
         ...
         patch("hermes_cli.main._resolve_node_runtime_npm", side_effect=_fake_resolve_node_runtime_npm):
        yield
