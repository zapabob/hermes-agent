...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        if managed_path:
            ...
            if not managed_hit:
                # ``shutil.which`` observes PATHEXT on Windows, but a managed
                # portable Node tree can be inspected before its shim is made
                # visible to the probe process.  Probe the concrete, runnable
                # shim names as a final managed-tree fallback.
                candidate_names = (
                    ("agent-browser.cmd", "agent-browser.exe", "agent-browser")
                    if os.name == "nt"
                    else ("agent-browser",)
                )
                for path_entry in managed_path.split(os.pathsep):
                    if not path_entry:
                        continue
                    for candidate_name in candidate_names:
                        candidate = Path(path_entry) / candidate_name
                        if candidate.is_file():
                            managed_hit = str(candidate)
                            break
                    if managed_hit:
                        break
