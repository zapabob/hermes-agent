...
``hermes_cli/worktree_gc.py`` for the policy and shared invariants). Usage:
    ...
    hermes worktree prune            # print a plan; never mutates by default
    hermes worktree prune --yes      # apply the displayed reclaim plan
...
def cmd_worktree(args) -> int:
    ...
    if action == "prune":
        ...
        apply = bool(getattr(args, "yes", False) or getattr(args, "apply", False))
        # Reclaim is destructive. A bare command is deliberately plan-only;
        # --yes/--apply is the explicit consent token.
        ...
        if actions:
            ...
            if plan_only:
                print("Plan only: rerun with --yes (or --apply) to reclaim these paths.")
        else:
