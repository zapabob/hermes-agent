...
`HERMES_TEST_WORKERS` now resolves to 96 for `NousResearch/hermes-agent` and 4 for the fork. The official repository path is unchanged, while the fork avoids oversubscribing the standard hosted runner.
...
- A fresh exact-SHA CI run is required after commit and push.
