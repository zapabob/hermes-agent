...
_IDENTICAL_RESULT_REFERENCE_PREFIX = (
    "[hermes note: this result is byte-identical to the "
)
_IDENTICAL_RESULT_REFERENCE_MARKER = " (tool_call_id "
_IDENTICAL_RESULT_REFERENCE_SUFFIX = (
    "). Refer to that result; it has not changed. Args: "
)
...
def identical_result_reference_call_id(result: Any) -> str | None:
    """Return the original call id carried by an identical-result stub.

    Context compression uses this small parser to keep the referenced full
    payload reachable when it rewrites an older prefix.  Keep it beside the
    stub builder so the producer and consumer cannot silently drift apart.
    Ordinary tool output is never interpreted as a reference unless it starts
    with Hermes' exact generated prefix and carries the generated marker.
    """
    ...
    marker_at = first_line.find(_IDENTICAL_RESULT_REFERENCE_MARKER)
    if marker_at < 0:
        return None
    call_id_start = marker_at + len(_IDENTICAL_RESULT_REFERENCE_MARKER)
    call_id_end = first_line.find(")", call_id_start)
    if call_id_end < 0:
        return None
    if not first_line.startswith(
        _IDENTICAL_RESULT_REFERENCE_SUFFIX, call_id_end
    ):
        return None
    call_id = first_line[call_id_start:call_id_end].strip()
    ...
    def _check_loop_cap(
        ...
        if tool_name == "web_search":
            ...
            if cap and self._turn_web_search_count >= cap:
                ...
                # Do not set _halt_decision: block != hard halt (fork policy).
                return decision
