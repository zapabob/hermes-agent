...
def _messages_match_scoped_identity(left: Any, right: Any) -> bool:
    """Compare the live turn identity used for rotation stamping."""
...
def _join_cancelled_worker(future: Any, grace_seconds: float) -> bool:
    ...
    try:
        ...
        # ``future.result`` also re-raises a TimeoutError produced by the
        # worker itself. In that case the future is already settled and no
        # provider call remains in flight; only a still-pending future means
        # the bounded join actually expired.
        return future.done()
