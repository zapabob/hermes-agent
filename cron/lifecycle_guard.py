...
def contains_gateway_lifecycle_command(text: str) -> bool:
    ...
    token_modes = (True, False) if os.name == "nt" else (True,)
    for preserve_windows_paths in token_modes:
        for segment in _iter_command_segments(
            normalized,
            preserve_windows_paths=preserve_windows_paths,
        ):
            joined = " ".join(segment)
...
def _iter_command_segments(
    command: str,
    *,
    preserve_windows_paths: bool = True,
) -> Iterator[list[str]]:
    ...
    for line in logical_lines:
        ...
        try:
            ...
            if os.name == "nt" and preserve_windows_paths:
...
def _resolve_terminal_script_path(candidate: str, cwd: Optional[str]) -> Optional[Path]:
    ...
    if path is None:
        return None
    if not path.is_absolute() and not PurePosixPath(candidate).is_absolute():
        try:
    ...
    *,
    ...
    for script_path in _iter_referenced_shell_scripts(command, cwd=cwd):
        ...
        if unsafe:
            return True
        loaded_remote = False
        remote_path = script_path.as_posix()
        if script_text is None and read_remote_script is not None:
            ...
            loaded_remote = script_text is not None
            if unsafe:
                return True
        if not script_text:
            continue
        ...
        if loaded_remote and PurePosixPath(remote_path).is_absolute():
            script_dir = str(PurePosixPath(remote_path).parent)
        else:
