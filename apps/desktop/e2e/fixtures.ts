...
export function writeMockProviderConfig(
  ...
  modelIds: string[] = ['mock-model'],
): void {
  const configPath = path.join(hermesHome, 'config.yaml')
  const configuredModels = modelIds.length > 0 ? modelIds : ['mock-model']
  const modelEntries = configuredModels.map(model => `      ${model}: {}`).join('\n')
...
async function closeDesktop(app: ElectronApplication): Promise<void> {
  ...
  const closed = await Promise.race([
    ...
    new Promise<boolean>(resolve => {
      gracefulTimer = setTimeout(() => resolve(false), 10_000)
    }),
  ])
  if (gracefulTimer) {
  ...
  await Promise.race([
    closePromise,
    new Promise<boolean>(resolve => {
      forcedTimer = setTimeout(() => resolve(false), 5_000)
    }),
  ])
  if (forcedTimer) {
