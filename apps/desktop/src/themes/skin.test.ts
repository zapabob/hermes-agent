...
describe('skinToDesktopTheme', () => {
  ...
  it('falls back from invalid wallpaper presentation values', () => {
    const theme = skinToDesktopTheme({
      name: 'wallpaper',
      colors: { background: '#101010', ui_text: '#f0f0f0' },
      background_image: 'C:/Hermes/skins/wallpaper.png',
      background_image_fit: 'url(javascript:alert(1))',
      background_overlay: 'linear-gradient(red, blue)'
    })!

    expect(theme.backgroundImageFit).toBe('cover')
    expect(theme.backgroundOverlay).toBeUndefined()
  })
})
