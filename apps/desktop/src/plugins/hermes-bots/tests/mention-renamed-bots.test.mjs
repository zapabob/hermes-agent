...
test('botMentionTag: friendly collisions fall back to unique handles for roster and group completion', () => {
  const { botMentionTag } = runtime({ meta: { local: { title: 'Research Buddy' } } })
  const roster = [
    { name: 'local', title: 'Research Buddy', connectionId: 'local' },
    { name: 'remote', handle: 'remote-mac-mini', display_name: 'Research Buddy', connectionId: 'mac-mini', remoteSource: true },
    { name: 'third', ui_meta: { 'hermes-bots': { title: 'Research Buddy' } }, connectionId: 'linux-box', remoteSource: true }
  ]

  assert.equal(botMentionTag(roster[0], roster), 'local')
  assert.equal(botMentionTag(roster[1], roster), 'remote-mac-mini')
  assert.equal(botMentionTag(roster[2], roster), 'third')
})
...
test('resolveRosterMentions: friendly collisions stay ambiguous across local and remote bots', () => {
  const { resolveRosterMentions } = runtime({ meta: { local: { title: 'Research Buddy' } } })
  const live = { name: 'default', connectionId: 'local' }
  const roster = [
    { name: 'local', title: 'Research Buddy', connectionId: 'local' },
    { name: 'remote', handle: 'remote-mac-mini', display_name: 'Research Buddy', connectionId: 'mac-mini', remoteSource: true },
  ...
  // The slug, collapsed spelling, and a case-normalized title are all the
  // same identity. None may choose a last writer after two or three matches.
  for (const text of ['@research-buddy', '@researchbuddy', '@RESEARCH-BUDDY']) {
    assert.deepEqual(JSON.parse(JSON.stringify(resolveRosterMentions(text, roster, live))), [])
  }

  // Profile handles remain independent, unique aliases after a friendly-name
  // collision, including the remote disambiguated handle.
  assert.deepEqual(JSON.parse(JSON.stringify(resolveRosterMentions('@local', roster, live).map(b => b.name))), ['local'])
  assert.deepEqual(JSON.parse(JSON.stringify(resolveRosterMentions('@remote-mac-mini', roster, live).map(b => b.name))), ['remote'])
})
...
test('parseGroupChatMentions: title/display collisions are fail-closed for two and three members', () => {
  const { parseGroupChatMentions, groupMemberKey } = runtime()
  const members = [
    { name: 'local', title: 'Site Smith', connectionId: 'local' },
    { name: 'remote', handle: 'remote-mac-mini', display_name: 'Site Smith', connectionId: 'mac-mini', remoteSource: true },
    { name: 'third', ui_meta: { 'hermes-bots': { title: 'Site Smith' } }, connectionId: 'linux-box', remoteSource: true }
  ]

  for (const text of ['@site-smith', '@sitesmith']) {
    const parsed = parseGroupChatMentions(text, members)
    assert.equal(parsed.mentioned.size, 0)
  }

  // A member's own profile handle remains a safe unique route.
  const unique = parseGroupChatMentions('@remote-mac-mini', members)
  assert.deepEqual([...unique.mentioned], [groupMemberKey(members[1])])
})
...
test('composer autocomplete offers the renamed tag and matches on the display name', () => {
  ...
  // A friendly tag shared by local and remote profiles is not offered as a
  // dead candidate; both completion surfaces fall back to resolver-safe
  // canonical handles.
  roster.profiles = [
    { name: 'default' },
    { name: 'writer', display_name: 'Research Buddy' },
    { name: 'remote', handle: 'remote-mac-mini', display_name: 'Research Buddy', connectionId: 'mac-mini', remoteSource: true },
    { name: 'third', display_name: 'Research Buddy', connectionId: 'linux-box', remoteSource: true }
  ]
  const collision = provide('res')
  assert.deepEqual(JSON.parse(JSON.stringify(collision.map(i => i.insert))), ['@writer', '@remote-mac-mini', '@third'])
})
