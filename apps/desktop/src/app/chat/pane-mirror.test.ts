...
function setup(options: {
  anchor?: (tile: Tile) => string | undefined
  dir?: (tile: Tile) => 'center' | 'left' | 'right' | undefined
...
describe('paneMirror workspace scope', () => {
  ...
  it('refreshes dock placement and workspace ownership together', () => {
    const mirror = setup({
    ...
    mirror.source.set([{ anchor: 'workspace', dock: 'right', id: 'one', owner: 'connection-a::default' }])
    expect(mirror.contribution('one')).toMatchObject({
      data: { dock: { pane: 'workspace', pos: 'right' } },
      workspaceOwnerKey: 'connection-a::default'
    })

    mirror.source.set([{ anchor: 'session:two', dock: 'center', id: 'one', owner: 'connection-b::default' }])
    expect(mirror.contribution('one')).toMatchObject({
      data: { dock: { pane: 'session:two', pos: 'center' } },
