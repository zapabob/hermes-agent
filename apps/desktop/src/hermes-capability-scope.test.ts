...
describe('capability helpers are connection-scoped', () => {
  ...
  it('object scopes pin every read and write to the named connection', () => {
    ...
    void addMcpServer(
      { name: 'filesystem', url: 'https://example.test/mcp' },
      { connectionId: 'homelab', profile: 'inbox-bot' }
    )
    void removeMcpServer('filesystem', { connectionId: 'homelab', profile: 'inbox-bot' })
    void setMcpServerEnabled('filesystem', true, { connectionId: 'homelab', profile: 'inbox-bot' })
    void getMcpCatalog({ connectionId: 'homelab', profile: 'inbox-bot' })
    void authMcpServer('filesystem', { connectionId: 'homelab', profile: 'inbox-bot' })
    void getMcpOAuthFlow('flow-1', { connectionId: 'homelab', profile: 'inbox-bot' })
    void cancelMcpOAuthFlow('flow-1', { connectionId: 'homelab', profile: 'inbox-bot' })
    void installMcpCatalogEntry('filesystem', {}, { connectionId: 'homelab', profile: 'inbox-bot' })
