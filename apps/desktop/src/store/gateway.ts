...
export function setPrimaryGateway(
  ...
  profile = 'default',
  connectionId: null | string | undefined = undefined
): void {
  ...
  if (!gateway) {
    ...
    g.primaryConnectionId = normConnectionId(connectionId)
  }
...
export function gatewayForScope(scope: GatewayScope): HermesGateway | null {
  ...
  const secondary =
    g.secondaries.get(registryBackendScopeKey(normalized.connectionId, normalized.profile))?.gateway ?? null
