...
// Compatibility value for the fixed image-fetch guard in main.ts. The
// user-configurable local-file cap continues to use dataUrlReadMaxBytesFromMb
// at each IPC call; this alias only preserves the established default image
// limit while the shared module owns the MB bounds.
const DATA_URL_READ_MAX_BYTES = dataUrlReadMaxBytesFromMb(DATA_URL_READ_DEFAULT_MAX_MB)
