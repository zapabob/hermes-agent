...
const NOW = 1_000_000
const FRESH_MS = resolvePoolKeepaliveFreshMs(undefined)
...
test('resolves the production keepalive tolerance and bounded override', () => {
  assert.equal(resolvePoolKeepaliveFreshMs(undefined), 4 * 60_000)
  assert.equal(resolvePoolKeepaliveFreshMs('300000'), 300_000)
  assert.equal(resolvePoolKeepaliveFreshMs('1'), 120_000)
})
