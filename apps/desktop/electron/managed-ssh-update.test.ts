...
test('POSIX managed launcher executes the updater command and atomically publishes its status', async () => {
  const localHome = await mkdtemp(path.join(os.tmpdir(), 'hermes-managed-launch-'))
  const home = posixTestPath(localHome)
  ...
  try {
    ...
    await rm(localHome, { force: true, recursive: true })
  }
})
...
posixObserverTest('POSIX observer reads the exact correlation receipt and terminal marker from disk', async () => {
  const localHome = await mkdtemp(path.join(os.tmpdir(), 'hermes-managed-update-'))
  const home = posixTestPath(localHome)

  try {
    const receipts = path.join(localHome, 'logs', 'update_receipts')
    await mkdir(receipts, { recursive: true })
    await writeFile(path.join(localHome, `.update_exit_code.${CORRELATION}`), '0')
    await writeFile(
...
posixObserverTest('managed observer unwraps a named profile home for the install-wide marker', async () => {
  const localRoot = await mkdtemp(path.join(os.tmpdir(), 'hermes-managed-profile-marker-'))
  const localProfileHome = path.join(localRoot, 'profiles', 'research')
  const profileHome = posixTestPath(localProfileHome)
