...
$WatchdogDataDir = if ($env:HERMES_WATCHDOG_DATA) {
    $env:HERMES_WATCHDOG_DATA
} elseif ($env:LOCALAPPDATA) {
    Join-Path $env:LOCALAPPDATA "HermesWatchdog"
} else {
    Join-Path $HermesHome "watchdog-go"
}
$WatchdogMaintenancePath = Join-Path $WatchdogDataDir "maintenance.json"
...
function Set-WatchdogMaintenanceState([string]$State, [string]$Reason) {
    try {
        New-Item -ItemType Directory -Path $WatchdogDataDir -Force -ErrorAction Stop | Out-Null
        if (Test-Path -LiteralPath $WatchdogMaintenancePath) {
            $existing = Get-Content -LiteralPath $WatchdogMaintenancePath -Raw -Encoding UTF8 | ConvertFrom-Json
            if ($script:WatchdogMaintenanceHeld) {
                if ($existing.owner -ne $script:WatchdogMaintenanceOwner -or $existing.nonce -ne $script:WatchdogMaintenanceNonce) {
                    throw "watchdog maintenance ownership changed"
                }
            } elseif ($existing.state -ne "NORMAL") {
                $existingExpiry = [DateTimeOffset]::MinValue
                if (-not [DateTimeOffset]::TryParse([string]$existing.leaseExpiresAt, [ref]$existingExpiry) -or $existingExpiry -gt [DateTimeOffset]::UtcNow) {
                    throw "watchdog maintenance is already owned by $($existing.owner)"
                }
            }
        }
        $now = [DateTimeOffset]::UtcNow
        $expires = if ($State -eq "NORMAL") { $now } else { $now.AddHours(4) }
        $payload = [ordered]@{
            schemaVersion = 1
            state = $State
            owner = $script:WatchdogMaintenanceOwner
            nonce = $script:WatchdogMaintenanceNonce
            epoch = $script:WatchdogMaintenanceEpoch
            timestamp = $now.ToString("o")
            reason = $Reason
            leaseSeconds = 14400
            leaseExpiresAt = $expires.ToString("o")
            pid = $PID
            processStartTime = ([DateTimeOffset]::new((Get-Process -Id $PID).StartTime.ToUniversalTime()).ToUnixTimeMilliseconds() / 1000.0)
            repoRoot = $InstallRoot
        }
        $tmp = Join-Path $WatchdogDataDir (".maintenance.json.{0}.{1}.tmp" -f $PID, [Guid]::NewGuid().ToString("N"))
        ...
        if (Test-Path -LiteralPath $WatchdogMaintenancePath) {
            [System.IO.File]::Replace($tmp, $WatchdogMaintenancePath, $null)
        } else {
        ...
        $env:HERMES_WATCHDOG_MAINTENANCE_EPOCH = [string]$script:WatchdogMaintenanceEpoch
        Write-HandoffLog "watchdog maintenance state=$State"
    } catch {
        Write-HandoffLog "ERROR: could not publish watchdog maintenance state ${State}: $($_.Exception.Message)"
        throw
    } finally {
...
function Wait-WatchdogMaintenanceAcknowledge {
    ...
    throw "live watchdog did not acknowledge maintenance within 35s"
}
...
try {
    ...
    # The external Go watchdog is the sole automatic outer recovery authority.
    # During this planned handoff it must observe only: Desktop exit, gateway
    # drain, venv mutation, and the official relaunch all belong to this script
    # and its ``hermes update`` child until NORMAL is published again.
    Set-WatchdogMaintenanceState "UPDATE_PREPARE" "Desktop updater is preparing an owned handoff"
    Wait-WatchdogMaintenanceAcknowledge

    # -- 1. Wait for the Desktop to exit (FAIL CLOSED) ----------------------
    Set-WatchdogMaintenanceState "UPSTREAM_DRAIN" "Official Desktop updater owns process shutdown"
    ...
    if (-not $script:TreeSafeToFinalize) {
        ...
        if ($script:WatchdogMaintenanceHeld) {
            try {
                Set-WatchdogMaintenanceState "RECOVERY" "Official Desktop updater is restoring and relaunching"
            } catch {}
        }
        ...
        if ($script:WatchdogMaintenanceHeld) {
            try {
                Set-WatchdogMaintenanceState "NORMAL" "Official Desktop update recovery complete"
            } catch {}
        }
    }
}
