...
def _shell_words_at(command: str, start: int) -> list[str]:
    ...
    for _ in range(64):
        ...
        if words and "\n" in command[cursor:word_start]:
            break
        words.append(_deobfuscate_guard_word(raw_word))
        cursor = word_end
    return words


def _deobfuscate_guard_word(raw_word: str) -> str:
    """Preserve native Windows path separators before shell deobfuscation."""
    if os.name == "nt":
        prefix, separator, value = raw_word.partition("=")
        candidate = value if separator else prefix
        if len(candidate) >= 2 and candidate[0] == candidate[-1] and candidate[0] in {"'", '"'}:
            candidate = candidate[1:-1]
        if _WINDOWS_ABSOLUTE_PATH_RE.match(candidate):
            return f"{prefix}{separator}{candidate}" if separator else candidate
