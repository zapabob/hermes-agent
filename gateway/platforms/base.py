...
def _warn_unresolved_docker_media(candidate: PurePosixPath, session_key: str, reason: str) -> None:
...
def _translate_docker_container_media_path(candidate: PurePosixPath, session_key: str = "") -> Optional[Path]:
    ...
    # holds the file.
    matched: List[Tuple[Path, PurePosixPath, int]] = []
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    container_candidate = PurePosixPath(expanded_raw) if expanded_raw.startswith("/") else None
    translated = (
        _translate_docker_container_media_path(container_candidate, session_key=session_key)
